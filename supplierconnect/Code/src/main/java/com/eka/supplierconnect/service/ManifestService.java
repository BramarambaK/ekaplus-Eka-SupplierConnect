...
public class ManifestService {
	...
	public Attributes getManifestAttributes(){
	    ...
	    try {
			...
			throw new ConnectException(commonService.getErrorMessage("SC035", "Unable to locate manifest file", "supplierconnect"), fno);
		} catch (IOException ioe) {
			logger.error(Logger.EVENT_FAILURE, ioe.getLocalizedMessage(), ioe);
			throw new ConnectException(commonService.getErrorMessage("SC036", "Unable to read Manifest file", "supplierconnect"), ioe);
		}
	    return atts;	    		
	}
